export class PersonDataViewService {
    ...
    public addToList(index: number): Array<any> {
      ...
      list = this.addNameFieldIfNotPresent(list);
  ...
  private addNameFieldIfNotPresent(list: Array<any>) {
        return list.map(e => {
            if(e._type == 'payees' && !e.hasOwnProperty('_isHeaderRow')){
              e.name = e.payToAccountName;
            }else if(e._type == 'contacts' && !e.hasOwnProperty('_isHeaderRow')){
             e.name = e.customer.firstName;
            }
            return e;
        });
  }
